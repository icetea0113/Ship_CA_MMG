...
**************************")
    ...
    if ship_type_str == "2":
        δ_list, u_list, v_list, r_list, x_list, y_list, ψ_list = mmg_equations_KLVCC2.zigzag_test_mmg_3dof(
            ship_type,
            target_δ_rad,
        ...
        δ_list, u_list, v_list, r_list, x_list, y_list, ψ_list = mmg_equations_S175.zigzag_test_mmg_3dof(
            ship_type,
            target_δ_rad,
            target_ψ_rad_deviation,
            time_list,
    ...
    # df["angle"] = list(map(lambda psi: psi * 180 / np.pi, ship.psi))
    # df.to_csv("zigzag.csv", index=False)

    plt.plot(time_list , ship.F_N, color="limegreen")
