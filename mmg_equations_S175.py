...
def MotionEquation(ship: mmg.Ship, npm, u, v, r, δ):
    ...
    du = ((m + m_y) * v * r + X_R + X_P + X_H) / (m + m_x)

    dv = (-(m + m_x) * u * r + Y_R + Y_H) / (m + m_y)

    dr = (N_R + N_H) / (ship.principal_dimensions.I_zz + J_x)
...
def HullForcesEquation_no_Added_mass(ship: mmg.Ship, u, v, r):
    global ρ
    U = np.sqrt(u**2 + v**2)
    beta = 0.0 if U == 0.0 else np.arctan2(-v,u)
    ...
    X_H = 0.5 * ρ * ship.principal_dimensions.Lpp * ship.principal_dimensions.d * (U ** 2) * (
        ship.maneuvering_params.X_vv * (beta ** 2) +
        ship.maneuvering_params.X_rr * (r_dash ** 2) +
        ship.maneuvering_params.X_vr * beta * r_dash
    ) - R

    Y_H = 0.5 * ρ * ship.principal_dimensions.Lpp * ship.principal_dimensions.d * (U ** 2) * (
        ship.maneuvering_params.Y_v * beta +
        ship.maneuvering_params.Y_r * beta +
        ship.maneuvering_params.Y_vvv * (beta ** 3) +
        ship.maneuvering_params.Y_vvr * (beta ** 2) * r_dash +
        ship.maneuvering_params.Y_vrr * beta * (r_dash ** 2) +
        ship.maneuvering_params.Y_rrr * (r_dash ** 3)
    )

    N_H = 0.5 * ρ * ship.principal_dimensions.Lpp ** 2 * ship.principal_dimensions.d * (U ** 2) * (
        ship.maneuvering_params.N_v * beta +
        ship.maneuvering_params.N_r * r_dash +
        ship.maneuvering_params.N_vvv * (beta ** 3) +
        ship.maneuvering_params.N_vvr * (beta ** 2) * r_dash +
        ship.maneuvering_params.N_vrr * beta * (r_dash ** 2) +
...
def Propeller_RudderForcesEquation(ship: mmg.Ship, npm, u, v, r, δ, state = 0):
    ...
    β = 0.0 if U == 0.0 else np.arctan2(-v, u)
    β_p = β - ship.principal_dimensions.x_P * r_dash
    w_P = ship.principal_dimensions.w_P0 * np.exp(ship.principal_dimensions.C_1* (β_p ** 2))
